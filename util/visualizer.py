...
class Visualizer():
    def __init__(self, opt):
        ...
        if self.use_html:
            self.web_dir = os.path.join(opt.checkpoints_dir, opt.run_name, 'web')
            self.img_dir = os.path.join(self.web_dir, 'images')
            print('create web directory %s...' % self.web_dir)
            util.mkdirs([self.web_dir, self.img_dir])
        self.log_name = os.path.join(opt.checkpoints_dir, opt.run_name, 'loss_log.txt')
        with open(self.log_name, 'a+') as log_file:
            now = time.strftime("%c")
            log_file.write('Network: %s\nConfiguration: %s\n================ Training Loss (%s) ================\n' % (opt.net_name, opt.run_name, now))
